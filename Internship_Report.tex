...
%   \begin{center}
%     \vspace*{1.5cm}
%     {\Large \textbf{IASc-INSA-NASI}} \\[0.3cm]
%     {\large Summer Research Fellowship Programme --- 2026} \\[1.5cm]

%     \rule{\linewidth}{0.4pt} \\[0.4cm]
%     {\LARGE \bfseries PCBRE: A Multi-Modal Hardware Auditing Framework} \\[0.4cm]
%     \rule{\linewidth}{0.4pt} \\[1.5cm]

%     {\large \textit{Technical Internship Report}} \\[0.2cm]
%     {\normalsize Submitted in partial fulfillment of the requirements for the Indian Academy of Sciences (IASc) Summer Research Fellowship} \\[1.5cm]

%     \textbf{Candidate:} \\[0.2cm]
%     {\large Seikh Souvagya Mustakim} \\[0.1cm]
%     B.Tech Electronics and Telecommunication Engineering \\
%     Application No.: ENGS5100 \\[1.2cm]

%     \textbf{Under the Guidance of:} \\[0.2cm]
%     {\large Dr. Haresh Dagale} \\[0.1cm]
%     Department of Electronic Systems Engineering (DESE) \\
%     Indian Institute of Science (IISc), Bengaluru \\[1.5cm]
%   \end{center}
% \end{titlepage}
...
% \subsection{The Hardware Assurance Imperative in Global Supply Chains}
% \label{subsec:hw_assurance}
...
% \subsection{The Shift from Destructive to Non-Destructive Reverse Engineering}
% \label{subsec:nd_re}
...
% %%  SECTION 2: DATASET
% %% ============================================================
% \section{Dataset Curation and Analysis}
% \label{sec:dataset}

% \subsection{Roboflow Dataset Assembly and Class Distribution}
% \label{subsec:roboflow_dataset}

% The primary training dataset was sourced from the Roboflow Universe, using the publicly accessible \textit{Component Detection} dataset (\texttt{component-detection-caevk}, Version 1). This dataset is specifically curated for object detection tasks on populated PCB images and includes oriented bounding box annotations in YOLOv8-OBB format, enabling direct compatibility with the training framework employed in this work.
...
% \subsection{Literature Review: Early Heuristic Thresholding Models (Johnson, 2014)}
...
% \subsection{Diagnostic Bottlenecks of Rule-Based Segmentation}
% \label{subsec:stage1_limits}
...
% Despite its role as a foundational preprocessing step, the Stage 1 HSV masker exhibits three structural failure modes that motivate the subsequent shift to deep learning detection in Stage 3.

% \textbf{First}, the HSV threshold boundaries require manual calibration per board. Different manufacturers apply solder masks with slightly different dye formulations, causing the characteristic $H$ values of nominally ``green'' boards to vary by up to $\pm 15$ hue units. Boards with red (e.g., Chinese-manufactured development boards) or purple solder masks are entirely outside the default configuration, requiring the user to re-tune sliders.

% \textbf{Second}, false positives from high-reflectance solder joints are common. Solder alloys in reflow-soldered joints exhibit broad specular highlights that map into the low-saturation, high-value region of HSV space, overlapping with the IC package epoxy mask. Aggressive erosion kernels can reduce these false positives but simultaneously fragment the boundaries of genuine IC body detections.

% \textbf{Third}, and most critically, the Stage 1 output provides only a spatial mask --- it does not produce semantic class labels. The method cannot distinguish between an IC package, a large ceramic capacitor, a transformer, or a mechanical heatsink attachment. All downstream semantic classification relies entirely on the Stage 3 deep learning detector described in Section~\ref{sec:stage3}.


% %% ============================================================
% %%  SECTION 4: STAGE 2
% %% ============================================================
% \section{Stage 2: OCR Preprocessing (Classical Computer Vision)}
% \label{sec:stage2}

% \subsection{Contrast Limited Adaptive Histogram Equalization (CLAHE) Implementation}
% \label{subsec:clahe}
...
% Standard global histogram equalization redistributes pixel intensities uniformly across the full image range. While effective on images with globally uniform illumination, it fails on IC crop images because the chip package body is a small, dark epoxy region that is typically surrounded by bright metallic pads. Global equalization over-amplifies the pad brightness while under-utilizing the contrast range within the dark chip body where the text resides.

% Contrast Limited Adaptive Histogram Equalization (CLAHE) resolves this limitation by partitioning the image into an $8 \times 8$ grid of non-overlapping tiles and performing independent histogram equalization within each tile. Let $h_t(v)$ denote the intensity histogram of tile $t$ at luminance value $v$. CLAHE clips the histogram at a user-defined contrast limit $c_{\text{lim}}$ before redistribution, preventing over-amplification of homogeneous regions:
...
% The clipped excess is uniformly distributed across all histogram bins before mapping. In the PCBRE implementation, $c_{\text{lim}} = 2.0$ is used, providing a moderate contrast enhancement that recovers character legibility without amplifying photon noise in underexposed regions.
...
% \subsection{Gaussian Unsharp Masking for High-Frequency Edge Retention}
% \label{subsec:unsharp}
...
% where $\alpha \in [1.0, 2.0]$ is the sharpening strength coefficient and $\sigma = 1.0$ is the Gaussian standard deviation. Physically, a larger $\alpha$ amplifies character stroke boundaries more aggressively; however, values exceeding 2.0 introduce ringing artifacts (Gibbs phenomenon) at sharp edges that degrade the token sequence consistency of downstream OCR decoders. The implementation applies Equation~(\ref{eq:unsharp_mask}) to the CLAHE-processed image prior to OCR submission.

% \subsection{Literature Review: Tesseract OCR Limitations on PCBs (Kleber et al., 2017)}
...
% %%  SECTION 5: STAGE 3
% %% ============================================================
% \section{Stage 3: Deep Learning Object Detection (YOLOv8)}
% \label{sec:stage3}

% \subsection{The Paradigm Shift: From Classical Image Processing to Parameter Tuning}
% \label{subsec:paradigm_shift}

% The limitations catalogued in Sections~\ref{sec:stage1} and~\ref{sec:stage2} converge on a defining observation: classical, rule-based computer vision cannot generalize across the enormous variability in PCB form factors, component densities, solder mask colors, component rotation angles, illumination conditions, and board scale. For each new board configuration, a practitioner must manually retune HSV threshold bounds, contour area filters, aspect ratio constraints, and morphological kernel sizes --- a process that is not only time-consuming but fundamentally non-scalable.

% The solution lies in replacing the hand-engineered feature pipeline with a parametric deep learning object detector whose features are learned from annotated data. Deep Convolutional Neural Networks (CNNs), particularly those in the YOLO (You Only Look Once) family, operate by learning hierarchical spatial feature representations directly from labelled image-annotation pairs. Given sufficient training diversity, these networks generalize to unseen board configurations without manual re-tuning, offering a fundamentally superior automation baseline.
...
% where $(x_c, y_c)$ is the box centroid, $(w, h)$ are the box half-dimensions in the rotated frame, and $\theta \in (-90^{\circ}, 90^{\circ}]$ is the rotation angle relative to the horizontal axis. The YOLOv8-OBB architecture~\cite{jocher2023yolov8} extends the YOLOv8 detection head with an additional angular regression branch, enabling simultaneous prediction of location, size, class probability, and orientation in a single forward pass.

% \subsection{Network Architecture and Oriented Bounding Box Foundations}
% \label{subsec:obb_arch}

% YOLOv8 implements a CSPDarknet backbone with C2f (Cross-Stage Partial with 2 feature fusion) bottleneck modules. The neck employs a BiFPN-style Path Aggregation Network (PAN) to fuse feature maps from three scales (P3, P4, P5), enabling simultaneous detection of small passive components (at high-resolution P3 features) and large IC packages (at semantically rich P5 features). The detection head is decoupled, separating classification and regression branches, and operates in a distribution focal loss (DFL) framework that models bounding box edge uncertainty as a probability distribution rather than a point estimate.
...
% \subsection{Training Dynamics: Nano, Small, and Medium Model Scaling}
% \label{subsec:training_dynamics}

% Three model configurations were trained and evaluated. All models used the AdamW optimizer with a warmup phase over the first 3 epochs, cosine learning rate scheduling with a final learning rate of $lr_f = 0.01 \times lr_0$, and Mosaic+Mixup data augmentation. The VRAM constraint of the Tesla T4 GPU (16 GB) imposed practical limits on the batch size and resolution product:
...
% \textbf{YOLOv8n-OBB (Nano):} The Nano configuration trains in under 12 minutes on the T4 GPU and reaches an overall mAP@0.5 of 0.445 at 640-pixel resolution. The dominant failure mode is small-object feature resolution loss: at 640 pixels, a $0402$-size SMD resistor ($1.0 \times 0.5$~mm physical) occupies only $3 \times 6$ pixels in a macro photograph, which compresses to less than 1 pixel after three consecutive stride-2 downsampling operations in the backbone. The feature map representation of such components merges with background, preventing reliable detection.

% \textbf{YOLOv8s-OBB (Small):} Increasing the input resolution to 1024 pixels is the single most impactful architectural change. Resistor AP rises from 0.127 to 0.414 --- a 226\% relative improvement --- because the same physical resistor now occupies $5 \times 10$ pixels at intake, surviving backbone downsampling to a representable $1 \times 2$ pixel feature in the P3 map. Early stopping terminates training at epoch 117 (best weights at epoch 67), preventing late-stage overfitting observed in the Nano run.
...
% The YOLOv8m-OBB model stands out as the champion architecture among the three configurations for several key reasons. First, its expanded capacity of 26.4M parameters provides the necessary representation space to learn rich, orientation-invariant feature maps. While the Nano configuration suffers from sub-pixel information loss due to its low 640px resolution, and the Small configuration suffers from gradient saturation, the Medium model successfully balances local detail extraction with global contextual awareness. Second, YOLOv8m-OBB displays superior robustness against severe class imbalance. In a dataset where capacitors and resistors comprise over 93\% of all validation instances, smaller backbones are dominated by majority-class gradient signals during backpropagation. The Medium backbone's extra capacity allows it to preserve minority-class filters, resulting in a near-perfect AP of 0.995 for Diodes and 0.970 for Inductors. Third, the Medium model achieves a substantial mAP@0.5 boost to 0.749 (a 68.3\% relative increase over Nano and 31.6\% over Small) while maintaining a highly practical inference latency of 31.4~ms on the Tesla T4 GPU. This makes it ideal for hardware auditing where high localization precision is critical and sub-50~ms latency is completely acceptable.
...
% \subsection{Experimental Inference Results and Champion Model plots}
% \label{subsec:yolo_results}

% The optimal YOLOv8m-OBB configuration was subjected to diagnostic validation. Figure~\ref{fig:yolov8m_performance} houses the primary validation performance curves generated during execution on the Tesla T4 GPU.
...
%     \caption{Training and Validation Metrics (containing loss components and evaluation curves)}
%     \label{fig:yolov8m_loss}
%   \end{subfigure}
%   \\ \vspace{1em}
...
%   \item \textbf{Normalized Confusion Matrix (b):} Delineates inter-class leakage. Resistors are occasionally misclassified as Capacitors due to identical rectangular package outlines (e.g., 0402 footprints). Background confusion is low, validating the network's ability to segregate the green solder mask from components.
%   \item \textbf{Loss Curves (c):} Both classification and box coordinate regression losses converge monotonically. The validation loss tracks the training curves closely without a late-stage upward divergence, indicating the 50-epoch early stopping window successfully halted parameter updates before overfitting occurred.
% \end{itemize}

% \begin{figure}[!ht]
%   \centering
%   \includegraphics[width=0.8\textwidth]{{Internship Report assets/webapp_ss/Stage 3 (yolo component detector).png}}
%   \caption{Stage 3 YOLOv8m-OBB Inference Result on a Development Board: Coloured oriented bounding boxes delineate individual components. ICs are highlighted in blue, capacitors in green, and resistors in orange. Note the tight, non-overlapping box fits around rotated 0402 passive components.}
...
%   \caption{Stage 3 Inference on a Raspberry Pi Model B board: The detector successfully localizes the BCM2835 SoC, SMSC LAN9512 controller, Micron DRAM package, and passive arrays around the board edge. This evaluation demonstrates generalization beyond the Roboflow training distribution.}
%   \label{fig:stage3_rpi}
% \end{figure}

% \subsubsection{Error Analysis: Resistor False Negatives and Transistor Imbalance}
% \label{subsec:error_analysis}

% The Resistors class deserves targeted diagnostic attention. With 401 validation instances --- the second largest class by count --- an mAP@0.5 of only 0.581 and a recall of 0.432 indicates that the medium model misses approximately 57\% of resistors in the validation set. This is a structurally motivated failure, not a parameter tuning artifact. At $1024 \times 1024$ input resolution, a standard $0402$-package SMD resistor ($1.0 \times 0.5$~mm physical dimensions) maps to approximately $5 \times 10$ pixels in a macro photograph. After the YOLOv8 backbone applies three stride-2 convolutions (total downsampling factor $2^3 = 8$), the feature map representation of this resistor occupies less than one pixel at the P3 detection scale. Convolutional feature aggregation over sub-pixel regions is unable to separate the resistor's characteristic rectangular electrode signature from the background solder mask.

% For the Transistors class (6 validation instances), the mAP@0.5 of 0.431 reflects a different pathology: class-imbalance loss domination. In the cross-entropy training loss, the gradient magnitude contributed by each class is proportional to its instance count. With 401 resistor instances versus 6 transistor instances, the optimizer's parameter updates are dominated by resistor and capacitor gradient signals, resulting in insufficient weight tuning for the transistor feature representation. Focal loss weighting --- assigning higher loss multipliers to hard-to-classify minority classes --- could partially address this without requiring additional data.

% \subsection{IC Pin and Port Detection Sub-Models}
% \label{subsec:pin_port_models}

% \subsubsection{IC Pin Detection Model}
% The pin detection sub-model is trained to localize individual solder-pad pins on IC packages. This capability enables the pipeline to automatically populate the \texttt{pin\_count} field in the Hardware Bill of Materials (HBoM) JSON output, which serves as a vital index for downstream footprint and datasheet matching.

% The model was trained on the Kaggle \texttt{seikhmustakim/ic-pins} dataset containing 38 training and 10 validation images (157 instances) for 100 epochs. To resist overfitting on this data-scarce split, transfer learning with backbone freezing was employed, locking the first 10 layers (CSPDarknet backbone stem). The model achieved the following validated metrics on the best-weights checkpoint (epoch 98):
% \begin{itemize}
...
% A critical deployment constraint was discovered during evaluation: the model performs optimally when invoked on the full-board image rather than individual cropped IC segments. When cropped images are fed, the pins appear upscaled relative to the training scale distribution, causing detection confidence scores to collapse. The pipeline addresses this by executing global full-board inference, then spatially filtering the detected pin centroids within the boundaries of parent IC boxes returned by the primary component detector.
...
%   \begin{subfigure}[b]{0.35\textwidth}
%     \centering
%     \includegraphics[width=\linewidth]{pin_inference_IC_Package_C.png}
%     \caption{IC Package C Pin Detections}
%     \label{fig:pin_inf_c}
%   \end{subfigure}
%   \hfill
%   \begin{subfigure}[b]{0.35\textwidth}
%     \centering
%     \includegraphics[width=\linewidth]{pin_inference_IC_Package_D.png}
%     \caption{IC Package D Pin Detections}
%     \label{fig:pin_inf_d}
%   \end{subfigure}
%   \caption{IC Pin Detection Inference: Clean oriented bounding boxes (OBBs) localized on high-density IC packages (Packages A, B, C, and D) showing tight pad alignment and no label overlays.}
%   \label{fig:pin_inference}
% \end{figure}


% \subsubsection{Connector Port Detection Model}
% The physical connector port detector localizes and identifies external communication ports (USB Type-A, Micro-USB, Ethernet RJ-45, HDMI, Audio jacks, etc.) under the single aggregated class \texttt{Port-USB-Ethernet-etc}. These connectors define the physical attack surface of the board, making their cataloguing essential for hardware security threat modeling.

% The model was trained on the Roboflow \texttt{scmu/usb-ethernet-ps2-port} dataset containing approximately 26 training and 9 validation images (29 instances) for 50 epochs on an Intel Xeon CPU. Unlike the pin model, the port detector was trained without layer freezing. It achieved the following performance metrics on the best-weights checkpoint:
% \begin{itemize}
...
% \begin{figure}[!ht]
%   \centering
%   \includegraphics[width=0.8\textwidth]{port_inference_result.png}
%   \caption{Physical Port Detection Inference: Oriented bounding box localizing multiple adjacent I/O connectors on a physical board edge.}
%   \label{fig:port_inference}
% \end{figure}


% %% ============================================================
% %%  SECTION 6: HBOM GENERATION
% %% ============================================================
% \section{Multimodal IC Text Extraction and HBoM Generation}
% \label{sec:hbom}

% \subsection{Addressing the Lack of High-Quality OCR Training Data}
% \label{subsec:ocr_data_gap}

% Section~\ref{subsec:fics_pcb} established that no freely accessible, annotated IC text corpus is available for fine-tuning domain-specific OCR models. The ICText-AGCL dataset~\cite{ng2024ictext} remains the most complete academic resource for this task, providing character-level bounding box annotations on 10,000 IC marking images under diverse degradation conditions. However, the restricted access policy of this dataset, combined with the limited duration of the fellowship programme, makes supervised fine-tuning infeasible within the project timeline.

% Local general-purpose OCR engines were evaluated as baseline alternatives. EasyOCR employs a CRNN (Convolutional Recurrent Neural Network) architecture with a CTC decoder trained primarily on document and natural scene text corpora. PaddleOCR integrates a DB (Differentiable Binarization) text detection head with a SVTR (Scene Text Recognition Transformer) recognition module. Both systems demonstrate high accuracy on their target domains but fail systematically on IC package markings. The domain gap manifests in three specific failure modes:

% \begin{itemize}
%   \item \textbf{Truncation}: EasyOCR reads only the highest-confidence character block, yielding `STM32F' from a package marking that reads `STM32F745VGT6'. The suffix `745VGT6' is suppressed because the character sequence does not match any learned linguistic pattern in the document training corpus.
%   \item \textbf{Suffix Dropout}: PaddleOCR correctly reconstructs the base processor family (`STM32F745') but drops the package suffix `VGT6', which encodes the lead-free, 100-pin LQFP package variant information critical for footprint verification.
%   \item \textbf{Rotation Failure}: Both systems operate under the implicit assumption of near-horizontal text baselines. IC packages placed at $90^{\circ}$ or $270^{\circ}$ orientations produce unreadable null outputs from engines that lack explicit rotation normalization.
% \end{itemize}

% \subsection{Integrating the Gemini 1.5 Flash Large Multimodal Model (LMM)}
% \label{subsec:gemini_integration}

% The Gemini 1.5 Flash API~\cite{team2024gemini} is integrated as the primary IC text extraction engine. Gemini 1.5 Flash is a multimodal transformer model that processes images and text jointly within a unified attention architecture. The model was pre-trained on a large-scale multimodal corpus and exhibits strong zero-shot generalization to specialized image domains including industrial inspection, scientific microscopy, and electronic component photography.

% The integration is implemented as an exponential backoff retry loop with a maximum of 3 attempts, dispatching crops in batches of 3 images per API call. Batch size is constrained by the model's 1 Million Token Per Minute (TPM) limit on Google AI Studio free-tier accounts. Each IC crop is first downscaled to a maximum dimension of 250 pixels via bilinear interpolation, reducing the base64-encoded payload size by approximately 80\% while retaining sufficient character resolution for the model's vision encoder.

% \begin{figure}[!ht]
%   \centering
%   \includegraphics[width=0.8\textwidth]{{Internship Report assets/webapp_ss/Gnerated HboM full (all component).png}}
%   \caption{Stage 3 Automated HBoM Table: Gemini 1.5 Flash returns structured JSON objects for each detected IC. The table displays Part Number, Manufacturer, Logo/Brand, Confidence Score, Category, Package, Pins, and Datasheet links. The IC-only and full-component views are available as separate export targets.}
%   \label{fig:hbom_full}
% \end{figure}



% \subsection{Zero-Shot, Orientation-Invariant Text Recognition}
% \label{subsec:zero_shot_ocr}

% The core advantage of Gemini 1.5 Flash in this context is its ability to perform zero-shot, orientation-invariant text recognition. The model processes the IC crop image through a vision encoder that produces spatial feature embeddings, which are then cross-attended by the language decoder. This architecture enables reading of text at arbitrary orientations because the cross-attention mechanism learns to extract character sequences from any spatial arrangement of visual features, without requiring explicit geometric normalization.

% On the Schutzwerk STM32F745 evaluation board, Gemini 1.5 Flash correctly reads all 5 of the primary IC packages in the IC-only view, achieving 98-99\% confidence scores on correctly identified packages:
...
% \clearpage
% \subsection{Automated Structuring of the JSON Hardware Bill of Materials}
% \label{subsec:hbom_structure}
...
% The category and package fields are resolved by querying the LCSC free search API (\texttt{wmsc.lcsc.com/ftps/wm/product/search?keyword=\{part\_number\}}) with a fallback to AllDatasheet.com scraping. The complete HBoM is presented in the web interface as an interactive sortable table and exported as a CSV file on user request.


% %% ============================================================
% %%  SECTION 7: STAGE 4
% %% ============================================================
% \section{Stage 4: Probe-Free Power Rail Localization}
% \label{sec:stage4}

% \subsection{Theoretical Intuition of Euclidean Distance Transforms}
% \label{subsec:edt_theory}
...
% \subsection{Parameter Implementation: The 6.0px Thickness Threshold}
...
% The 6.0~px threshold employed in Stage 4 is deliberately conservative, accounting for lower-resolution input images (e.g., smartphone captures at 8 megawatts) and compressed JPEG encoding artifacts that degrade binary mask precision. Traces above this threshold are flagged as candidate power rail segments. A user-adjustable slider in the Stage 4 interface allows practitioners to calibrate this parameter to their specific imaging setup.
...
% \subsection{Net Node Extraction: X/Y Coordinate Mapping and IPC Rail Classification}
% \label{subsec:net_nodes}

% The Stage 4 pipeline executes in three coupled layers:

% \textbf{Layer 1 --- LMM Silkscreen Coordinate Extraction:} The full board image is submitted to Gemini 1.5 Flash with a structured power rail localization prompt requesting identification of silkscreen-printed power labels (``VCC'', ``GND'', ``3V3'', ``5V'', ``VBUS'', ``AGND'', ``PWR'') as percentage coordinates relative to the image dimensions. The model returns a JSON array of objects of the form $\{$\texttt{"text": "VCC", "type": "VCC", "x\_pct": 45.2, "y\_pct": 23.7}$\}$ This approach is motivated by the finding in VIPR-PCB~\cite{bhattacharyay2022vipr} that silkscreen PWR/GND labels are reliable power rail indicators that the model can locate from visual context.

% \textbf{Layer 2 --- Local CV Width Classification:} For each detected label coordinate, a $100 \times 100$~pixel patch is extracted from the board image centered on the reported location. The patch is converted to grayscale, enhanced with CLAHE, and binarized using Otsu's global threshold to produce the copper mask $\mathcal{B}$. OpenCV's \texttt{distanceTransform} function with \texttt{DIST\_L2} metric is applied, yielding the EDT field $D(p)$. Equation~(\ref{eq:trace_width}) is applied to compute the trace width. If the result exceeds 6.0~px, the node is confirmed as an IPC-classified power rail.

% \textbf{Layer 3 --- HBoM Regulator Coupling:} Part numbers returned by the Stage 3 HBoM generator are matched against a curated prefix table of known voltage regulator families: \texttt{AMS1117}, \texttt{LM78}, \texttt{LM33}, \texttt{LDO}, \texttt{TPS}, \texttt{MIC5}, \texttt{LTC}, \texttt{AP2112}. Any IC whose part number matches a known regulator prefix is cross-referenced with the Stage 3 bounding box coordinates and highlighted on the canvas with a yellow bounding box, confirming its role as a VCC supply source in the power topology.
...
% \subsection{Contact-Based Robotic Probing Methodologies}
...
% \subsection{CAD-Dependent and Graph-Based Verification Frameworks}
...
% \subsection{Methodological Classification and Comparative Summary}
...
% %% ============================================================
% %%  SECTION 9: DISCUSSION AND ARCHITECTURAL JUSTIFICATION
% %% ============================================================
% \section{Discussion and Architectural Justification}
% \label{sec:discussion}

% The modular, hybrid architecture of PCBRE combines local edge intelligence with cloud-based Large Multimodal Models (LMMs). This co-designed division of labor provides five distinct operational benefits:
...
%   \item \textbf{Geometric Imprecision and Hallucination:} Auto-regressive VLMs lack dedicated regression layers and NMS, leading to spatial hallucinations or coordinate drifts. YOLOv8m-OBB provides deterministic boundary regression $(x_c, y_c, w, h, \theta)$ suitable for CAD layout generation.
  
%   \item \textbf{Compute Efficiency and Bandwidth:} Running HSV segmentation and YOLO localization locally prunes out over 95\% of background board pixels. Transmitting only active component crops (approx. 5\% of total bytes) reduces transmission latency and cloud token transaction costs.
  
%   \item \textbf{Coupled Semantic and Physical Verification:} While LMMs locate text labels (``VCC'', ``GND'') semantically, they cannot calculate physical geometries. PCBRE matches semantic suggestions with local Euclidean Distance Transform (EDT) width measurements to validate power nets against physical IPC-2221 guidelines.
  
%   \item \textbf{Modularity and Offline Security:} To satisfy secure, air-gapped hardware audit criteria, the swappable modular API interface allows practitioners to replace the cloud LMM with a locally hosted quantized model (e.g., \texttt{Qwen2-VL-7B-Instruct} run via \texttt{llama.cpp}) without altering the core pipeline.
% \end{itemize}
...
% \subsection{Summary of Pipeline Efficacy and Automation Improvements}
...
% The LMM-driven IC text extraction module represents a practical resolution to the domain-specific OCR bottleneck that has persisted as an open problem since Kleber et al. documented a complete zero-success rate on their 2017 evaluation. Gemini 1.5 Flash achieves near-perfect read accuracy on the Schutzwerk evaluation board, correctly identifying all five primary IC packages including their full package suffix codes, which encode pin count and temperature range information critical for footprint verification.

% \subsection{Transitioning to Quantized, Local Vision-Language Models for Offline Security}
% \label{subsec:future_edge}
...
% The planned resolution is the integration of local, quantized vision-language models (VLMs) running entirely on edge hardware. Specifically, the Qwen2-VL-7B-Instruct model~\cite{wang2024qwen2vl}, quantized to 4-bit precision using the GGUF $Q4\_K\_M$ format via \texttt{llama.cpp}, provides comparable zero-shot OCR performance to Gemini 1.5 Flash on IC package imagery while operating fully offline on consumer-grade GPU hardware (NVIDIA RTX 3060 or better). The same function interface (\texttt{gemini\_ocr\_chip()}) can be replaced with (\texttt{local\_vlm\_ocr\_chip()}) without any modification to the remaining pipeline, due to the modular architecture implemented throughout Stage 3.
...
% \subsection{Automated Schematic and Power Pin Tracing Capabilities}
% \label{subsec:future_schematic}
...
% %% ============================================================
% %%  APPENDIX A: EXTENDED TRAINING ANALYTICS
% %% ============================================================
% \appendix
% \section{Extended Training Analytics and Model Loss Curves}
% \label{app:training_analytics}

% This appendix provides supplementary quantitative detail on the training dynamics observed across the three YOLOv8-OBB model configurations evaluated in this work. All training was conducted on Tesla T4 GPU accelerators provided by Google Colab and Kaggle Notebook environments under the \texttt{PYTORCH\_CUDA\_ALLOC\_CONF=expandable\_segments:True} memory configuration to prevent out-of-memory errors under the 16 GB VRAM constraint at batch size 4.

% \subsection*{A.1 Training Loss Components}
% The YOLOv8-OBB training loss reported per epoch comprises three components: \texttt{box\_loss} (CIoU regression loss on predicted OBB coordinates), \texttt{cls\_loss} (Varifocal classification loss), and \texttt{dfl\_loss} (Distribution Focal Loss on box edge uncertainty). Healthy training is characterised by monotonically decreasing loss values across all three components with validation metrics peaking before early stopping triggers. Divergence between training and validation loss after the patience threshold indicates overfitting.
...
% \subsection*{A.2 Inference Speed Benchmarks}
...
% \subsection*{A.3 YOLOv8n-OBB (Nano) Performance Metrics}
...
% \subsection*{A.4 YOLOv8s-OBB (Small) Performance Metrics}
...
\begin{titlepage}
  ...
  \begin{center}
    ...
    % FULFILLMENT STATEMENT & DEGREE/PROGRAMME
...
\section{Benchmark Datasets \& Public Availability}
\label{sec:datasets}
